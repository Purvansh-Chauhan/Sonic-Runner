...
if (rankValues[i]< bestScore){
    bestRank = rankGrades[i]; //setting the rank after going through the array of grades mentioned 
}
}
if (bestScore < currentScore){ // if the player sets the higher score than the current best score than this looop will run
...
k.add([
  ...
  const bestRankBox = k.add([
    k.rect(400, 400, { radius: 4 }), // setting a rectangular box for the rank whose size will be 4
    k.color(0, 0, 0),// sets the color to black
    k.area(),
    k.anchor("center"),
    k.outline(6, k.Color.fromArray([255, 255, 255])), // sets the outline white and the dimensions of the borders of the rankbox would be as mentioned in this line of code
